...
public class TempBanCommand implements ICommand {
    ...
    public void execute(CommandEvent event, String args) {
        ...
        event.getMessage().getMentionedUsers().forEach(user -> {
            ...
            cases.addProperty("tempban", cases.get("tempban").getAsInt() + 1);
        ...
        event.reply("The Ban Hammer has spoken! Banned " + (String.join(", ", event.getMessage().getMentionedUsers().stream().map(Utils::getUser).collect(Collectors.toList()))) + " for " + sTime + ".").queue();
    }
    @Override
